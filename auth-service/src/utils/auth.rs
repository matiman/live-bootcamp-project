...
pub fn generate_auth_token(email: &Email) -> Result<String, TokenError> {
    let delta =
        chrono::Duration::try_seconds(TOKEN_TTL_SECONDS).ok_or(TokenError::UnexpectedError)?;
    ...
    let exp: usize = exp.try_into().map_err(|_| TokenError::UnexpectedError)?;

    let sub = email.as_ref().to_owned();

    let claims = Claims { sub, exp };

    create_token(&claims).map_err(TokenError::TokenError)
}
